...
def baked_goods_all():
    if request.method =='GET':
        baked_goods=[]
        for bakes in BakedGood.query.all():
            bakes_dict=bakes.to_dict()
            baked_goods.append(bakes_dict)
        ...
        db.session.add(new_bakes)
        db.session.commit()
        bakes_dict=new_bakes.to_dict()
        response=make_response(bakes_dict,201)
